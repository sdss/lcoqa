...
# All the functions in this file written by R. Owen except TransRotModel.
...
class TransRotScaleModel(Model):
    ...
    def basicApply(self, coeffs, posArr):
        ...
        rotMatTransposed = numpy.array(((coeffs[3], coeffs[2]), (-coeffs[2], coeffs[3])),
                                       dtype=float)
...
class TransRotModel(Model):
    """Model translation and rotation (no scale)

    out x  =  c0  + (cos(c2)  -sin(c2))  *  in x
        y     c1    (sin(c2)   cos(c2))        y
            thus:
            translation = (c0, c1)
            rotation = c2
    ...
    def __init__(self, coeffs=(0.0, 0.0, 0.0)):
        ...
        The default coefficients are for no translation or rotation change.
    ...
    def basicApply(self, coeffs, posArr):
        ...
        rotMat = numpy.array([[numpy.cos(theta), -numpy.sin(theta)],
                              [numpy.sin(theta), numpy.cos(theta)]], dtype=float)

        rot = numpy.dot(posArr, rotMat.T)

        return coeffs[0:2] + rot
    ...
    def getTransRot(self):
        ...
        return (self._coeffs[0:2],
                self._coeffs[2] / RadPerDeg)

    def setTransRot(self, xyTrans, rotAng):
        """Set coefficients from translation, rotation and scale factor
        """
        rotAngRad = rotAng * RadPerDeg
        self._coeffs = (
...
if __name__ == "__main__":
    ...
    for inPos, desOutPos in (
        ...
        if not numpy.allclose(outPos, desOutPos):
            print('Apply failed on coeffs=%s, inPos=%s, desOutPos=%s outPos=%s' %
                  (trsModel.getCoeffs(), inPos, desOutPos, outPos))
